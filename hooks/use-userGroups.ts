...
const useUserGroups = create<Status>()((set, get) => ({
  ...
  removeUserGroup: (id: string) => set((state) => ({ userGroups: state.userGroups.filter((group) => group._id !== id) })),
  updateLastReadMessage: (data: Message) => set((state) => ({ userGroups: state.userGroups.map((group) => group._id === data._id ? { ...group, lastReadMessage: data } : group) })),
  ...
  updateLastMessage: (data: Message) => set((state) => ({
    userGroups: state.userGroups.map((group) => group.group._id === data.group ? { ...group, group: { ...group.group, lastMessage: data } } : group)
  }))
}))
